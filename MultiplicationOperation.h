...
class MultiplicationOperation : public Node<T, N...>
{
public:
	virtual Tensor<T, N...> evaluate(const std::vector<Tensor<T, N...>>& operands) const override
	{
		if (operands.size() != 2)
		{
			throw OperandCountMismatchException("Multiplication operation requires exactly two operands.");
		}
		...
		/*if (operand1.size() != operand2.size()) {
			throw DimensionsDismatchException("Operand dimensions do not match for multiplication.");
		}
